#ifndef __AOB_LIST_H__
#define __AOB_LIST_H__

#include<Windows.h>
#include<string>

// RemoveCRC
std::wstring AOB_Run_ManipulatePacket_Leave_VM[] = {
	L"68 FF 00 00 00 6A 00 6A 00 8B ?? ?? ?? ?? ?? 83 ?? ?? ?? 6A 03 FF 15",
};

std::wstring AOB_RenderFrame[] = {

	//VC9(BB後)
	//56 57 8B F9 8B 07 8B 48 1C
	//56 8B F1 8B 06 57 56 FF 50 1C

	//VC8(BB前) 56 8B ?? 8B ?? 57 56 FF ?? 1C 8B ?? 85 ?? 7D

	// v186.1
	L"56 8B ?? 8B ?? 57 56 FF ?? 1C 8B ?? 85 ?? 7D 0C 68 ?? ?? ?? ?? 56 57 E8 ?? ?? ?? ?? 8B C7 5F 5E C3",
	// v188.0
	L"56 57 8B F9 8B 07 8B 48 1C 57 FF D1 8B F0 85 F6 7D 0E 68 ?? ?? ?? ?? 57 56 E8 ?? ?? ?? ?? 8B C6 5F 5E C3",
	// JMS334 TMS157.2
	L"56 8B F1 8B 06 57 56 FF 50 1C",
};

std::wstring AOB_Run_RenderFrame_Leave_VM[] = {
	L"6A 01 FF 15 ?? ?? ?? ?? 8B ?? 08 83 ?? 00 75",
};


ULONG_PTR Offset_OnEnterField_Enter_VM[] = {
	0x14,
};

std::wstring AOB_OnEnterField[] = {
	// v188.0
	L"55 8B EC 83 EC 40 53 56 57 89 4D C8 8B 4D C8 E8 ?? ?? ?? ?? E9",
};

std::wstring AOB_OnEnterField_Leave_VM[] = {
	// v188.0
	L"E8 ?? ?? ?? ?? 85 C0 74 ?? E8 ?? ?? ?? ?? 89 45 E8 83 7D E8 00 74",
};

// DR_Check
std::wstring AOB_DR_Check[] = {
	// v331.0, vmp
	L"55 8B EC 81 EC F0 02 00 00 A1 ?? ?? ?? ?? 33 C5 89 45 FC 53 56 57 6A 00 E9",
	// MSEA v102
	L"55 8B EC 81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C5 89 45 FC 53 56 57 E9",
};


// RemoveHackShield
std::wstring AOB_HackShield_Init[] = {
	// v164.0
	L"55 8B EC 81 EC ?? ?? ?? ?? 53 8B D9 83 7B 10 00 0F 85 ?? ?? ?? ?? E8 ?? ?? ?? ?? 85 C0 74",
	// v186.1
	L"55 8B EC 81 EC ?? ?? ?? ?? 53 8B D9 8D 4B ?? 89 4D ?? E8 ?? ?? ?? ?? 85 C0 0F 85 ?? ?? ?? ?? E8 ?? ?? ?? ?? 85 C0",
	// v302.0, MSEA v102
	L"81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C4 89 84 24 ?? ?? ?? ?? 55 ?? 8B ?? 8D ?? ?? 8B CD E8 ?? ?? ?? ?? 85 C0 0F 85 ?? ?? ?? ?? E8 ?? ?? ?? ?? 85 C0 74",
	// v332.0
	L"81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C4 89 84 24 ?? ?? ?? ?? 55 56 6A 00 8B F1 E9",
};

std::wstring AOB_EHSvc_Loader_1[] = {
	// v186.1
	L"55 8B EC 81 EC ?? ?? ?? ?? 57 C7 45 ?? 00 00 00 00 C6 85 ?? ?? ?? ?? 00 B9 ?? ?? ?? ?? 33 C0 8D BD ?? ?? ?? ?? F3 AB",
};

std::wstring AOB_EHSvc_Loader_2[] = {
	// v164.0
	L"55 8B EC 81 EC ?? ?? ?? ?? 57 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C6 85 ?? ?? ?? ?? 00 B9 82 00 00 00 33 C0 8D BD ?? ?? ?? ?? F3 AB C7 45 ?? 00 00 00 00 90 E8 ?? ?? ?? ?? 50 E8",
	// v164.0 call nop
	L"55 8B EC 81 EC ?? ?? ?? ?? 57 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C6 85 ?? ?? ?? ?? 00 B9 82 00 00 00 33 C0 8D BD ?? ?? ?? ?? F3 AB C7 45 ?? 00 00 00 00 E8 ?? ?? ?? ?? 90 50 E8",
	// v186.1
	L"55 8B EC 81 EC ?? ?? ?? ?? 57 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C6 85 ?? ?? ?? ?? 00 B9 ?? ?? ?? ?? 33 C0 8D BD ?? ?? ?? ?? F3 AB",
	// v334.2
	L"55 8B EC 81 EC ?? ?? ?? ?? 57 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C6 85 ?? ?? ?? ?? 00 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 C7 45 ?? 00 00 00 00 B9 09 00 00 00 33 C0 8D 7D ?? F3 AB",
};

std::wstring AOB_HeartBeat[] = {
	// v186.1
	L"B8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 81 EC ?? ?? ?? ?? 56 BE ?? ?? ?? ?? 56 8D 85 ?? ?? ?? ?? 6A 00 50 E8",
};

std::wstring AOB_MKD25tray[] = {
	// v164.0
	L"55 8B EC 83 EC 0C 56 8B F1 83 7E 18 00 0F 85 ?? ?? ?? ?? 83 65 FC 00 8D 45 FC 50 90 E8 ?? ?? ?? ?? 83 7D FC 00 59",
	// v164.0 call nop
	L"55 8B EC 83 EC 0C 56 8B F1 83 7E 18 00 0F 85 ?? ?? ?? ?? 83 65 FC 00 8D 45 FC 50 E8 ?? ?? ?? ?? 90 83 7D FC 00 59",
	// v176.0
	L"55 8B EC 83 EC 0C 56 8B F1 83 7E 18 00 0F 85 ?? ?? ?? ?? 83 65 FC 00 8D 45 FC 50 FF 15",
	// v186.1
	L"55 8B EC 83 EC ?? 56 8B F1 57 8D 7E ?? 8B CF E8 ?? ?? ?? ?? 85 C0 0F 85",
	// v302.0
	L"83 EC 0C 56 8B F1 57 8D 7E 30 8B CF E8 ?? ?? ?? ?? 85 C0 0F 85 A1 00 00 00 89 44 24 08 8D 44 24 08 50 FF 15",
	// v332.0
	L"83 EC 0C 56 57 6A 00 8B F1 E9",

};

std::wstring AOB_Autoup[] = {
	// v164.0
	L"56 8B F1 83 7E 14 00 74 16 68 ?? ?? ?? ?? 68 80 00 00 00 90 E8 ?? ?? ?? ?? 83 66 14 00 59 59 5E C3",
	// v164.0 call nop
	L"56 8B F1 83 7E 14 00 74 16 68 ?? ?? ?? ?? 68 80 00 00 00 E8 ?? ?? ?? ?? 90 83 66 14 00 59 59 5E C3",
	// v176.0
	L"56 8B F1 83 7E 14 00 74 ?? 68 ?? ?? ?? ?? 68 80 00 00 00 FF 15",
	// v186.1
	L"56 8D 71 ?? 8B CE E8 ?? ?? ?? ?? 85 C0 74 ?? 68 ?? ?? ?? ?? 68 ?? ?? ?? ?? FF 15",
	// v302.0
	L"6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC 0C 55 56 57 A1 ?? ?? ?? ?? 33 C4 50 8D 44 24 1C 64 A3 00 00 00 00 8B F1 89 74 24 10 C7 06 ?? ?? ?? ?? 8D 7E 18 8B CF C7 44 24 24 07 00 00 00 E8",
};

std::wstring AOB_ASPLunchr[] = {
	// v164.0
	L"55 8B EC 83 EC 0C 56 8B F1 83 7E 14 00 75 ?? 68 ?? ?? ?? ?? 90 E8",
	// v164.0 call nop
	L"55 8B EC 83 EC 0C 56 8B F1 83 7E 14 00 75 ?? 68 ?? ?? ?? ?? E8 ?? ?? ?? ?? 90",
	// v176.0
	L"55 8B EC 83 EC 0C 56 8B F1 83 7E 14 00 75 ?? 68 ?? ?? ?? ?? FF 15",
	// v186.1
	L"55 8B EC 83 EC ?? 56 8B F1 57 8D 7E ?? 8B CF E8 ?? ?? ?? ?? 85 C0 75 ?? 68 ?? ?? ?? ?? FF 15",
	// v302.0
	L"83 EC 0C 56 8B F1 57 8D 7E 24 8B CF E8 ?? ?? ?? ?? 85 C0 75 5B 68 ?? ?? ?? ?? FF 15",
	// v332.0
	L"83 EC 0C 56 57 6A 00 8B F1 E9",
};

std::wstring AOB_HSUpdate[] = {
	// v164.0
	L"55 8B EC 81 EC ?? ?? ?? ?? 53 8B D9 83 7B 0C 00 56 57 0F 85 ?? ?? ?? ?? 8A 15",
	// v180.1
	L"55 8B EC 81 EC ?? ?? ?? ?? 53 56 8D 59 ?? 57 8B CB E8 ?? ?? ?? ?? 85 C0 0F 85 ?? ?? ?? ?? 8A 15",
	// v186.1
	L"B8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 81 EC ?? ?? ?? ?? 56 83 C1 ?? 57 89 4D ?? E8 ?? ?? ?? ?? 85 C0 0F 85",
	// v187.0
	L"B8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 81 EC ?? ?? ?? ?? 53 56 83 C1 ?? 57 89 4D ?? E8 ?? ?? ?? ?? 85 C0 0F 85",
	// v302.0, MSEA v102
	L"6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 81 EC ?? ?? ?? ?? A1 ?? ?? ?? ?? 33 C4 89 84 24 ?? ?? ?? ?? 53 55 56 57 A1 ?? ?? ?? ?? 33 C4 50 8D 84 24 ?? ?? ?? ?? 64 A3 00 00 00 00 8D 69 0C 8B CD E8 ?? ?? ?? ?? 85 C0 0F 85",
};

// CSecurityClient::IsInstantiated
std::wstring AOB_EasyRemoveAntiCheat[] = {
	// v334.2
	L"33 C0 39 ?? ?? ?? ?? ?? 0F 95 C0 C3 CC CC CC CC C7 01 ?? ?? ?? ?? C3",
	// TMS157.2
	L"33 C0 39 ?? ?? ?? ?? ?? 0F 95 C0 C3 8B ?? ?? 04 85 C0 74 05 83 ?? 0C EB 02 33 C0",
	// TMS191
	L"83 ?? ?? ?? ?? ?? 00 0F 95 C0 C3 A1 ?? ?? ?? ?? C3 8B 01 8B",
	// TMS192.2
	L"83 ?? ?? ?? ?? ?? 00 0F 95 C0 C3 8B 44 24 04 85 C0 74 05 83 C0 0C EB 02 33 C0",
};

std::wstring AOB_StartKeyCrypt[] = {
	// v334.2
	L"83 ?? 0C 56 57 6A 00 8B ?? E9",
	// TMS157.2
	L"55 8B EC 83 EC 0C 56 8B F1 57 8D ?? 38 8B CF E8",
};

std::wstring AOB_StopKeyCrypt[] = {
	// JMS334.2 TMS157.2
	L"56 8B ?? 57 8D ?? 38 8B ?? E8 ?? ?? ?? ?? 85 ?? 74 ?? 8B",
};



std::wstring AOB_MapleNetwork[] = {
	// v194.0
	L"6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 53 56 A1 ?? ?? ?? ?? 33 C4 50 8D 44 24 ?? 64 A3 00 00 00 00 8B F1 8A 5C 24 ?? 8A 44 24 ?? 88 5E ?? 88 46 ?? 80 FB 01 75",
};

// FixClient
std::wstring AOB_WindowMode_PreBB[] = {
	// v164.0
	L"C7 45 E4 10 00 00 00 E8 ?? ?? ?? ?? 8D 45",
	// v186.1
	L"C7 45 DC 10 00 00 00 6A 03 FF 75 ?? 8D 4D ?? E8",
};

std::wstring AOB_WindowMode_PostBB[] = {
	// v187.0
	L"8B 45 ?? 89 45 ?? 6A 03 FF 75",
};

std::wstring AOB_Launcher[] = {
	// v186.1
	L"55 8B EC 83 EC ?? 53 56 57 33 DB 53 FF 15 ?? ?? ?? ?? 8B 7D ?? 89 3D ?? ?? ?? ?? 8B 87",
	// v188.0
	L"83 EC ?? 56 57 33 F6 56 FF 15 ?? ?? ?? ?? 8B 7C 24 ?? 89 3D ?? ?? ?? ?? 8B 87 ?? ?? ?? ?? 6A 65 56",
	//TMS forget version
	L"55 8B EC 83 EC 60 53 56 57 33 FF 57 FF ?? ?? ?? ?? ?? 8B 75 08 89",
	L"55 8B EC 81 EC 10 02 00 00 80 A5 F0 FD FF FF 00 8D 85 F0 FD FF FF 68",
};

std::wstring AOB_Ad[] = {
	// v186.1
	L"B8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 EC ?? 53 56 57 33 DB 53 FF 15",
	// v188.0
	L"6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC 64 53 55 56 57 A1 ?? ?? ?? ?? 33 C4 50 8D 44 24 78 64 A3 00 00 00 00 33 FF 57 FF 15",
};

#endif